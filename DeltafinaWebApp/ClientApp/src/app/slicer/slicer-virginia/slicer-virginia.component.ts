...
@Component({
  selector: 'slicer-virginia', // tslint:disable-line
  templateUrl: './slicer-virginia.component.html',
  styleUrls: ['slicer-virginia.css'],
})
export class SlicerVirginiaComponent implements OnInit {// implements AfterViewInit, OnDestroy {
  isRTL: boolean;
  ...
  // Istanza di linea: il template e' identico a quello del Burley, cambia solo
  // il device che questo getter restituisce.
  get slicer() {
    return DeviceService.slicerList.slicerVirginia;
  }
  ...
  // VSL_State_ON (0=off, 1=on) e VSL_Mode_AUTO (0=man, 1=AUT).
  ...
  onFlowRateSetpointConfirm(): void {
    ...
    try { this.userService.logParameterTagValues("SLICER VIRGINIA: setpoint portata oraria", s.SET_FLOW_RATE, this.newFlowRateSetpoint, "kg/h", this.appService.user); } catch (e) { }
  ...
  // Totalizzatore (V79) e numero casse (V90) sono i due contatori che l'operatore azzera
  ...
  onResetTotalizerAndBoxCount(): void {
    ...
    if (s.FDB_TOTALIZER != null) {
      try { this.userService.logParameterTagValues("SLICER VIRGINIA: reset totalizzatore", s.FDB_TOTALIZER, 0, "kg", this.appService.user); } catch (e) { }
      s.TOTALIZER = 0;
    }

    if (s.FDB_BOX_COUNT != null) {
      try { this.userService.logParameterTagValues("SLICER VIRGINIA: reset numero casse", s.FDB_BOX_COUNT, 0, "", this.appService.user); } catch (e) { }
      s.BOX_COUNT = 0;
    }
  }
  // #endregion
  ...
  // qui gli si passa solo la linea, cosi' la pagina Virginia mostra le sole casse del VSL.
  openWeighingsDialog(): void {
    const id: string = "WEIGHINGS_VSL";
    ...
    const dialogRef = this.dialog.open(SlicerWeighingPlateComponent, {
      ...
      maxWidth: '95vw',
      panelClass: 'plate-dialog',
      data: { lineCode: "VSL", line: "SLICER VIRGINIA" },
      hasBackdrop: false
    });
  }
  // #endregion
  ...
  // #region Finestra produzione - modo di taglio (C1500 del DB120 - FROM_HMI)
  ...
  readonly cutModeRow: SettingRow = settingByCode(SETUP_GROUP_SLICER, "C1500");
