...
@Component({
  selector: 'zona-2-3', // tslint:disable-line
  templateUrl: './zona-2-3.component.html',
  styleUrls: ['zona-2-3.css'],
})
export class Zona23Component implements OnInit {
  ...
  async ngOnInit() {
    ...
    [Zona23Component.CONVEYOR_ANIM,
     Zona23Component.CONVEYOR_ANIM_REV,
     Zona23Component.CYLINDER_ANIM].forEach(u => { var i = new Image(); i.src = u; });
  }
  ...
  getConveyorImg(name: string): string {
    ...
    if (motor == null || motor.STATE == null || !motor.RUNNING)
      return Zona23Component.CONVEYOR_OFF;
    if (motor.FDB_REVERSE != null && motor.FDB_REVERSE.value)
      return Zona23Component.CONVEYOR_ANIM_REV;
    if (motor.OUT_ALARM != null && motor.OUT_ALARM)
      return Zona23Component.CONVEYOR_ALARM;
    return Zona23Component.CONVEYOR_ON; // default: avanti (FDB_FORWARD o nessun feedback di verso)
  }

  getCylinderImg(motorName: string): string {
    var motor = this.findMotor(motorName);
    if (motor != null && motor.STATE != null && motor.RUNNING)
      return Zona23Component.CYLINDER_ANIM;
    return Zona23Component.CYLINDER_STATIC;
  }
  ...
  // ZONA 3.4 FINAL METAL DETECTOR"
  goToZone3_4(): void {
    this.router.navigate(['/zona-3-4']);
  }

  // "SILOS DISCHARGE" = sinottico di scarico silos (la scritta portava al Final Dryer)
